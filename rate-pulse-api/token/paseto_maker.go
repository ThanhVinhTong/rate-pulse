...
func NewPasetoMaker(symmetricKey string) (Maker, error) {
	if len(symmetricKey) < chacha20poly1305.KeySize {
		return nil, fmt.Errorf("symmetric key must be at least %d characters long", minSecretKeySize)
	}
	maker := &PasetoMaker{
		paseto:       paseto.NewV2(),
...
func (maker *PasetoMaker) CreateToken(userID int32, username, email, userType string, duration time.Duration) (string, *Payload, error) {
