"""Send Gmail notifications about pulse_intel runs (optional, env-driven)."""
...
def deliver_run_notifications(*, log_path: str | None, success: bool) -> None:
    ...
    smtp_user = os.getenv("GMAIL_SMTP_USER_INTEL", "").strip()
    app_pw = os.getenv("GMAIL_APP_PASSWORD_INTEL", "").strip()
    raw_to = os.getenv("GMAIL_NOTIFY_TO_INTEL", "").strip()
    ...
    if smtp_user and app_pw and raw_to:
        ...
        subject = f"pulse_intel run {status}"
        lines = [
            f"Run {status}.",
            "",
